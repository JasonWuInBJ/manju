...
export async function POST(request: Request, { params }: Props) {
  ...
  console.log('[Character Image] 开始生成图片', {
    characterId,
    aspectRatio,
  ...
  if (!prompt) {
    console.log('[Character Image] 错误: 缺少 prompt')
  ...
  try {
    ...
    console.log('[Character Image] 调用 RunningHub API', {
      apiUrl,
      hasApiKey,
    ...
    console.log('[Character Image] API 响应状态', {
    ...
    console.log('[Character Image] API 响应数据', {
      status: data.status,
      taskId: data.taskId,
    ...
    if (data.status === 'FAILED') {
      console.error('[Character Image] 图片生成失败', {
        errorMessage: data.errorMessage,
        failedReason: data.failedReason,
        fullResponse: data,
      })
    ...
    if (data.status === 'QUEUED' || data.status === 'RUNNING') {
      console.log('[Character Image] 任务进行中', {
        status: data.status,
        taskId: data.taskId,
      })
    ...
    if (!imageUrl) {
      console.error('[Character Image] 未获取到图片 URL', {
    ...
    console.log('[Character Image] 图片生成成功', {
      imageUrl,
      usage: data.usage,
    })
    ...
    console.log('[Character Image] 数据库更新成功', { characterId, imageUrl })
    ...
    console.error('[Character Image] 异常错误', {
